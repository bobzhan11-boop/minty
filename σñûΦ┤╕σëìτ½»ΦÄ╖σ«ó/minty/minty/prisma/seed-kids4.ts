// Additive, idempotent seed for the 27 new kids bags (HJ-BK200064..090) into the
// existing dev database — same product/keyword logic as bag-data.ts. Existing data is
// untouched. Run: npx tsx prisma/seed-kids4.ts
...
async function main() {
  ...
  const newRecs = catalog.filter((r) => /^HJ-BK2000(6[4-9]|[78][0-9]|90)$/.test(r.sku) && r.status === "labeled");
